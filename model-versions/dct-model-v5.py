# Cross-GAN test: Train on Stable Diffusion, Test on StyleGAN1 (flickr)
...
# Train - nested (Stable Diffusion)
print("Building training dataset (Stable Diffusion)...")
train_real, train_fake = load_images_nested(TRAIN_REAL_DIR, TRAIN_FAKE_DIR, MAX_IMAGES)
X_train, y_train = build_features(train_real, train_fake, "(Stable Diffusion)")
print(f"Train shape: {X_train.shape}")

# Test - flat (StyleGAN1)
print("Building test dataset (StyleGAN1)...")
test_real, test_fake = load_images_flat(TEST_REAL_DIR, TEST_FAKE_DIR, MAX_IMAGES)
X_test, y_test = build_features(test_real, test_fake, "(StyleGAN1)")
...
# Plot
folder_path = experiments_folder("dct-model-v5-experiment")
...
plt.ylabel('Accuracy')
plt.title('Cross-GAN: Stable Diffusion → StyleGAN1')
plt.legend()
plt.grid(True)
